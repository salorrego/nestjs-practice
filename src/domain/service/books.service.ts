...
export class BooksService {
  ...
  async getAllBooks(): Promise<BookModel[]> {
    try {
      ...
      Logger.error(`BooksService: Something went wrong while getting all books, error ${error.message}, stack ${error.stack}`)
  ...
  async createBook(book: CreateBookRequest): Promise<BookModel> {
    ...
    try {
      ...
      Logger.error(`BooksService: Something went wrong while saving book ${book.name}, error ${error.message}, stack ${error.stack}`)
      throw new HttpException(
